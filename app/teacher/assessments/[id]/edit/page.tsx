...
export default function TeacherEditAssessmentPage() {
  ...
  useEffect(() => {
    fetch(`/api/teacher/assessments/${id}`).then(async (res) => {
  ...
  if (notFound) return (
    <main className="p-6 text-center">
      <p className="text-red-600 mb-2">You can only edit your own submissions.</p>
      <Link href="/teacher/assessments" className="text-ns-blue">← Back to My Assessments</Link>
    </main>
  );
    ...
    <main className="min-h-screen bg-ns-cream p-4 pb-24">
      <div className="max-w-2xl mx-auto">
        <div className="flex justify-between items-center mb-4">
          <h1 className="text-2xl font-extrabold text-ns-purple">Edit My Assessment</h1>
          <div className="flex gap-3 text-sm">
            <Link href={`/report/${id}`} className="text-ns-blue">View Report</Link>
            <Link href="/teacher/assessments" className="text-ns-blue">← My Assessments</Link>
          </div>
        </div>
