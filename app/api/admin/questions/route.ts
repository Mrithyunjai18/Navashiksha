...
export async function POST(req: NextRequest) {
  ...
  const { label, type, options, points, parentQuestionId, triggerOption } = await req.json();
  ...
  if (parentQuestionId && !triggerOption?.trim())
    return NextResponse.json({ error: 'Select which answer should trigger this sub-question.' }, { status: 400 });
  ...
  await appendRow('CustomQuestions', {
    ...
    parentQuestionId: parentQuestionId || '', triggerOption: triggerOption || '',
  });
